...
class PointsToValidate extends React.Component {
  ...
  async getNoVPoints(){
    ...
    let response = await fetch(this.props.ApiPath+"points/getnonvalidated", requestOptions);
  ...
  async validatePoint(idPoint){
    ...
    let response = await fetch(this.props.ApiPath+"points/validate", requestOptions);
    let data = await response.json();
    console.log(data);
        ...
        this.state.points.forEach((element)=>{
            UI.push(
                <tr id={element.id}>
                    ...
                    <td><i>todo...</i></td> 
                    <td>
                    <button onClick={()=>{this.validatePoint(element.id)}}>✏️</button>
                    </td>
                </tr>
            );
        })
